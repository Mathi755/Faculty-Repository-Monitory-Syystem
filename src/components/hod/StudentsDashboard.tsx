...
import { Users, Target, BookOpen, Search } from "lucide-react";
...
const StudentsDashboard = () => {
  ...
  const Modal = () => {
    ...
    const facultyProjects = projects.filter(p => p.user_id === selectedFaculty.id);
          ...
          <div className="p-6 max-h-96 overflow-y-auto">
            {facultyProjects.length === 0 ? (
              <div className="text-center py-12">
                ...
                <div className="text-gray-500">No student projects found.</div>
              </div>
            ) : (
              <div className="space-y-4">
                {facultyProjects.map((proj) => (
                  <div key={proj.id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                    <div className="font-semibold text-gray-800 mb-2">{proj.project_title}</div>
                    <div className="space-y-1 text-sm text-gray-600">
                      <div><span className="font-medium">Type:</span> {proj.project_type}</div>
                      <div><span className="font-medium">Status:</span> {proj.status}</div>
                      <div><span className="font-medium">Semester:</span> {proj.semester || "N/A"}</div>
                      <div><span className="font-medium">Department:</span> {proj.department || "N/A"}</div>
                      <div><span className="font-medium">Created:</span> {new Date(proj.created_at).toLocaleDateString()}</div>
                      <div>
                        <span className="font-medium">Students:</span>
                        <ul className="list-disc list-inside ml-4">
                          {proj.students_involved.map((s, i) => (
                            <li key={i}>{s}</li>
                          ))}
                        </ul>
                      </div>
                      {proj.description && (
                        <div className="mt-2 text-xs text-gray-500">{proj.description}</div>
                      )}
                    </div>
                  </div>
                ))}
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50 min-h-screen">
      ...
      {/* Faculty Student Projects Contribution Overview */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-lg">Faculty Student Projects Contribution Overview</CardTitle>
                </div>
                <p className="text-sm text-gray-600 mt-1">Detailed view of all student projects guided by faculty</p>
              </div>
